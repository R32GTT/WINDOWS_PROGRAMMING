...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR:
		...
		else if (wParam == VK_BACK)
		{
			...
			GetCaretPos(&caretPos);
			std::wstring tef;
			/*if(!_text.wstr[_text.count-1] == NULL)
				tef = _text.wstr[_text.count-1];*/

			GetTextExtentPoint32W(hDC, reinterpret_cast<LPCWSTR>(&wParam), 1, &size);
			if (_text.count == 0 && _text.rcount > 0)
			{
				_text.rcount--;
				SetCaretPos(x + (20 * size.cx), caretPos.y - size.cy);
				_text.count = 20;
			}
		...
		else if (iswalpha(wParam) || wParam == VK_SPACE)
		{
